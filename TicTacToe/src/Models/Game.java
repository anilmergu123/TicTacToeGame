...
//            return ;
    ...
    public void undo(Game game) {
        ...
        Move lastMove = game.getMoves().get(size-1);
        game.getMoves().remove(size-1);
        //Make cellstate to Empty
        Board board = game.getBoard();
        Cell lastCell = board.getBoard().get(lastMove.getCell().getRow()).get(lastMove.getCell().getCol());
        lastCell.setCellstate(CellState.EMPTY);
        lastCell.setPlayer(null);
        for(WinningStrategy winningStrategy : game.winningStrategies) {
            winningStrategy.undo(lastMove,game.getBoard().getDimension());
        }
    }
